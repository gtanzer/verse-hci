...
let cmp_rule names (ity, in_name : ty * string) (t : ty) (o : Rule.t) : string =
...
let cmp_grammar (ity, oty, d : dsl) : string =
    ...
    let body = TMap.fold (fun t o ss -> (cmp_rule names (ity, in_name) t o)::ss) d [] in
    let footer = read_file (footerpath ^ "/grammar") in
    (String.concat "\n" ([header; input] @ body @ [footer]))
...
let cmp_dsl (ity, oty, d : dsl) : unit =
    let grammar = cmp_grammar (ity, oty, d) in
